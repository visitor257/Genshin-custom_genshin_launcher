...
#import sys
from os import listdir,startfile,chdir,getcwd,rename,remove,startfile,environ
    ...
    def mouseMoveEvent(self, event):
        ...
        if pass_accept==True:
            if self.eventtype_setting==False:
                if 0 <= y <= 41:
                    if 1245<=x<=1281 and self.eventtype_allenable==True:
                        if self.scfile_exist:
                        ...
                        if self.scfile_exist:
                            ...
                            self.win_con.setPixmap(QPixmap("custom_launcher_sources\\win_title_mini.png"))
                    elif 1172<=x<=1201 and self.eventtype_allenable==True:
                        if self.scfile_exist:
                ...
                if self.eventtype_setting==False:
                    if 0<=y<=41:
                        if 1245 <= x <= 1281 and self.eventtype_allenable==True:
                            ...
                            self.showMinimized()
                        elif 1172 <= x <= 1201 and self.start_bwiki.isVisible() and self.eventtype_allenable==True:
